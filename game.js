...
let SPRITES = {
    Tupa: "https://i.imgur.com/GKslMCA.gif",
    Sume: "https://i.imgur.com/kk8JdPf.gif",
    Caipora: "https://i.imgur.com/UMdh7bx.gif",
    ...
    ANHANGA: "https://i.imgur.com/bmCTfRc.gif", // NOVO BOSS FINAL
    
    Muiraquita: "https://i.imgur.com/WAWqKeB.png",
    VortexFX: "https://cdn-icons-png.flaticon.com/512/6631/6631998.png",
    ...
    // NOVAS ANIMAÇÕES DO BRAVELY CHAIN
...
let currentArcadeChallenge = 1; // 1 = primeiro desafio, 2 = segundo desafio
...
function showArcadeChallengeSelection() {
    ...
    // Verificar se o segundo desafio está disponível
    ...
    modal.innerHTML = `
        ...
        <div style="display: grid; grid-template-columns: ${secondChallengeAvailable ? '1fr 1fr' : '1fr'}; gap: 20px; width: 90%; max-width: 800px; margin-bottom: 30px;">
...
function showBestiaryModal() {
    ...
    if (savedData) {
        try {
            ...
            // Mostrar novos bosses se o primeiro desafio foi completado
            const firstChallengeBosses = ['SACI', 'MAPINGUARI', 'IARA', 'BOITATA'];
            const hasDefeatedFirstChallenge = firstChallengeBosses.every(boss => bossesDefeated.includes(boss));
            
            const newBossEntries = ['mulaEntry', 'corposecoEntry', 'lobisomemEntry', 'cucaEntry'];
            newBossEntries.forEach(entryId => {
...
function getRankingMetaString() {
    ...
    if(mode === "ARCADE") {
        const challenge = currentArcadeChallenge === 1 ? "first" : "second";
...
function populateFilterOptions() {
    ...
    if (bossFilter) {
        const bosses = [...new Set(RANKING_CACHE.map(e => e.boss))].filter(b => b && b !== "NONE" && b !== "PRIMEIRO" && b !== "SEGUNDO");
...
function applyRankingFilters() {
    ...
    if (filters.boss) {
        if (filters.mode === "COOP") {
            ...
            if (filters.boss === "PRIMEIRO" || filters.boss === "SEGUNDO") {
    ...
    if (mode === "BOSS" || mode === "ARCADE") {
        ...
        if (bossImg && !boss.dead) {
            ...
            if (alivePlayers.length > 0) {
                ...
                if (boss.type === "IARA" || boss.type === "CORPOSECO") {
                    if (shouldBossFlip) {
...
async function executeAction() {
    ...
    if(isBravely) {
        ...
        // ✅ CORREÇÃO: Apenas passar o turno se o boss ainda estiver vivo
        if (mode === "BOSS" || mode === "ARCADE") {
            isExecutingAction = false;
    ...
    // ✅ CORREÇÃO: Apenas passar turno se o jogo ainda estiver ativo
    if (gameActive) {
        manageTurns();
    }
...
function manageTurns() {
    ...
    if (mode === "PVP") {
        ...
        if(nextPlayerIdx < players.length) {
            ...
            addLog("👹 AGORA É O TURNO DO BOSS!");
...
// ================= NOVAS FUNÇÕES PARA OS NOVOS BOSSES =================
...
// ================= BOSSAI ATUALIZADO COM NOVOS BOSSES E ANHANGÁ =================
    ...
    if(!boss.dead) {
        ...
        if(boss.type === "BOITATA") { 
            ...
            if(Math.random() < 0.5) { 
                ...
                playSfx('saci'); 
                addLog("🌪️ Saci lança vórtices diagonais!");
                const diagonals = [
                    {dx:1, dy:1}, {dx:1, dy:-1}, 
                    {dx:-1, dy:1}, {dx:-1, dy:-1}
                ];
        ...
        // NOVOS BOSSES
            ...
            if(Math.random() < 0.6) { 
                ...
                directions.forEach(dir => { 
                    let nx = boss.x + dir.dx, ny = boss.y + dir.dy;
                    if(nx >= 0 && nx < 8 && ny >= 0 && ny < 8) {
                        triggerFireball(boss.x, boss.y, nx, ny);
            ...
            if(Math.random() < 0.5) { 
                playSfx('garras'); 
                addLog("💀 Corpo Seco sopra vapor podre em todas as direções (2 tiles)!");
                const directions = [
                    {dx: 0, dy: -1}, {dx: 0, dy: -2}, // Cima
                    {dx: 0, dy: 1}, {dx: 0, dy: 2},   // Baixo
                    {dx: -1, dy: 0}, {dx: -2, dy: 0}, // Esquerda
                    {dx: 1, dy: 0}, {dx: 2, dy: 0}    // Direita
                ];
                ...
                playSfx('garras'); 
                addLog("💀 Corpo Seco ataca com garras podres em 2 tiles!");
            ...
            if(Math.random() < 0.5) { 
                playSfx('garras'); 
                addLog("🐺 Lobisomem ataca com garras giratórias!");
                ...
                playSfx('minoa2'); 
                addLog("🐺 Lobisomem treme a terra e atrai os heróis!");
                triggerEarthquake();
                await sleep(800);
                ...
                // Atrair jogadores 1 tile em direção ao lobisomem
                players.forEach(p => { 
                    if(!p.dead) {
                        let dx = 0, dy = 0;
                        ...
                        // Prioridade: mover na diagonal em direção ao boss
                        if (dx !== 0 && dy !== 0) {
                            const nx = p.x + dx;
                            const ny = p.y + dy;
                            if (nx >= 0 && nx < 8 && ny >= 0 && ny < 8 && !isOccupied(nx, ny, p.id)) {
                                p.x = nx;
                                p.y = ny;
                            }
                        } else if (dx !== 0) {
                            const nx = p.x + dx;
                            if (nx >= 0 && nx < 8 && !isOccupied(nx, p.y, p.id)) {
                                p.x = nx;
                            }
                        } else if (dy !== 0) {
                            const ny = p.y + dy;
                            if (ny >= 0 && ny < 8 && !isOccupied(p.x, ny, p.id)) {
                                p.y = ny;
                            }
                        }
                    } 
                ...
                updateVisuals();
                addLog("🐺 Lobisomem atraiu os heróis para mais perto!");
            } 
        }
        else if(boss.type === "CUCA") { 
            if(Math.random() < 0.5) { 
                playSfx('cucask1'); 
                addLog("🧙‍♀️ Cuca lança uma poção venenosa em todo um quadrante!");
                const quadrant = triggerQuadrantPotionAttack();
                await sleep(600);
                
                // Aplicar dano a todos os jogadores no quadrante
                players.forEach(p => { 
                    if(!p.dead && 
                       p.x >= quadrant.xStart && p.x <= quadrant.xEnd &&
                       p.y >= quadrant.yStart && p.y <= quadrant.yEnd) {
                        applyDmg(p, bossAtk); 
                    }
                });
                
                // Aplicar dano reduzido aos tiles adjacentes
                for(let dx = -1; dx <= 1; dx++) {
                    for(let dy = -1; dy <= 1; dy++) {
                        if(dx === 0 && dy === 0) continue;
                        players.forEach(p => { 
                            if(!p.dead && 
                               p.x >= quadrant.xStart - 1 && p.x <= quadrant.xEnd + 1 &&
                               p.y >= quadrant.yStart - 1 && p.y <= quadrant.yEnd + 1 &&
                               !(p.x >= quadrant.xStart && p.x <= quadrant.xEnd &&
                                 p.y >= quadrant.yStart && p.y <= quadrant.yEnd)) {
                                applyDmg(p, 0); 
                            }
        ...
        // NOVO BOSS FINAL: ANHANGÁ
            ...
            if(Math.random() < 0.6) { 
                ...
                // Coluna do boss: ataque completo
                // Colunas laterais: sem ataque
    ...
    if (mode === "ARCADE") {
        ...
        if (arcadeIndex >= arcadeCurrentOrder.length) {
            ...
            if (boss.type === "ANHANGA") {
                ...
                addLog("🎉 ANHANGÁ FOI DERROTADO!");
                addLog("🏆 VOCÊ É O VERDADEIRO HERÓI DA FLORESTA!");
...
function saveAndRefresh() { 
    ...
    // Atualizar ambos os botões (os fixos acima do tabuleiro)
    document.querySelectorAll('#btnPlay').forEach(btn => {
        btn.disabled = !isMyTurn;
    });
    document.querySelectorAll('#btnSkill').forEach(btn => {
        btn.disabled = !isMyTurn || players[currentPlayerIdx]?.skillUsed;
    });
...
async function spawnArrowProjectile(f, t){ 
    const s = getStep(), a = document.createElement('img'); 
    a.src = SPRITES.ArrowFX; 
    a.className = 'arrow-fx'; 
    a.style.left = (f.x * s + 20) + 'px'; 
    a.style.top = (f.y * s + 20) + 'px'; 
    document.getElementById('board').appendChild(a); 
    
    await sleep(50);
    
    a.style.left = (t.x * s + 20) + 'px'; 
    a.style.top = (t.y * s + 20) + 'px'; 
